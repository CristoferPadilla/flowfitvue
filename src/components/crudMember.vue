  ...
  methods: {
    async savemember() {
      ...
      if (window.confirm(confirmMessage)) {
        if (this.selectedMember) {
          ...
          await this.registerAndSendMember();
        }
      }
    },
    ...
    async addmember() {
      try {
        ...
        const response = await axios.post("https://api-zydf.onrender.com/members", formData, {
          headers: {
    ...
    async registerAndSendMember() {
      try {
        const newMemberResponse = await this.addmember();

        const assignedMembership = this.memberships.find(membership => membership.id === this.newMember.assigned_membership);
        const membershipPrice = assignedMembership.price;
        const currentDate = new Date().toISOString().slice(0, 19).replace('T', ' ');

        const postData = {
          member_id: newMemberResponse.data.id,
          membership_id: this.newMember.assigned_membership,
          price: membershipPrice,
          sale_date: currentDate
        };

        const saleResponse = await axios.post("https://api-zydf.onrender.com/MembershipSale", postData, {
          headers: {
            Authorization: `Bearer ${this.token}`,
          },
        });

        console.log("Datos enviados correctamente a la API:", saleResponse.data);

        await this.fetchMembers();
        this.showForm = false;
        this.resetForm();
      } catch (error) {
        console.error("Error al registrar y enviar miembro y venta de membresía:", error);
      }
    },
    showAddForm() {
      this.showForm = true;
